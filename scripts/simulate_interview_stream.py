"""
Interview Simulation Script (Streaming SSE Version)

Simulates a complete interview session using the streaming SSE API by:
1. Starting an interview via /interview/start/stream
2. Using LLM to generate realistic candidate answers based on persona
3. Continuing via /interview/chat/{session_id}/stream until completion

Usage:
    python scripts/simulate_interview_stream.py [--base-url URL] [--api-key KEY] [--persona PERSONA] [--mode MODE]

Examples:
    # Local with default (predefined_questions mode, uses DEFAULT_QUESTION_SET_ID)
    python scripts/simulate_interview_stream.py

    # Staging with evasive persona
    python scripts/simulate_interview_stream.py --persona evasive

    # Run with dynamic_gap mode
    python scripts/simulate_interview_stream.py --mode dynamic_gap

    # Show only final results (no token streaming output)
    python scripts/simulate_interview_stream.py --quiet

    # List available personas
    python scripts/simulate_interview_stream.py --list-personas
...
class StreamingInterviewSimulator:
    """Simulates an interview session using streaming SSE API calls and LLM-generated answers."""
    ...
    def _consume_stream(self, method: str, url: str, payload: dict) -> dict:
        """
        Connect to an SSE endpoint, consume all events, and return the parsed result.

        Streams token events to stdout in real-time for visual feedback.
        Returns a dict with keys from the 'complete' event, or error info.
        """
        result = {}
        token_buffer = []
        event_count = 0
        ...
        try:
            with connect_sse(
                ...
                if response.status_code >= 400:
                    ...
                    self._log(f"[ERROR] HTTP {response.status_code}: {body}")
                ...
                for event in event_source.iter_sse():
                    ...
                    if event.event == "session":
                        ...
                        self._log(f"  [{data.get('node', '')}] {data.get('message', '')}")

                    elif event.event == "node":
                        self._log(f"  Node complete: {data.get('node')}")
                        ...
                        self._log(f"  Progress: {stage} - {detail}")

                    elif event.event == "complete":
                        if self.verbose and token_buffer:
                            print()  # newline after streamed tokens
                        result = data

                    elif event.event == "error":
                        self._log(f"[ERROR] Stream error: {data.get('detail', data)}")
                        result = {"error": data.get("detail", str(data))}

        except httpx.HTTPStatusError:
            raise
        except Exception as e:
            self._log(f"[ERROR] Stream connection failed: {e}")
            raise

        self._log(f"  (Received {event_count} SSE events)")
        return result
    ...
    def start_interview(self) -> dict:
        """Start a new interview session via streaming API."""
        self._log("\n" + "=" * 60)
        self._log("Starting Interview Simulation (Streaming)")
        ...
        self._log("\n[Starting interview stream...]")
        result = self._consume_stream(
            "POST",
            f"{self.base_url}/interview/start/stream",
            payload,
        )

        if "error" in result:
            raise RuntimeError(f"Failed to start interview: {result['error']}")

        return result

    def continue_interview(self, answer: str) -> dict:
        """Continue interview with candidate's answer via streaming API."""
        ...
        result = self._consume_stream(
            "POST",
            f"{self.base_url}/interview/chat/{self.session_id}/stream",
            payload,
        )

        if "error" in result:
            raise RuntimeError(f"Stream error: {result['error']}")
    ...
    def run(self) -> dict:
        ...
        current_question = start_response.get("question")

        if not current_question:
            self._log("[ERROR] No question received from start endpoint")
            raise RuntimeError("No question in start response")
        ...
        while self.answer_count < self.max_answers:
            ...
            # Send answer to streaming API
            self.answer_count += 1
            self._log(f"\n[Sending answer #{self.answer_count} to streaming API...]")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simulate an interview session using streaming SSE API with LLM-generated answers",
    ...
    simulator = StreamingInterviewSimulator(
